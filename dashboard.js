...
async function loadOtherPages() {
  ...
  usersSnap.forEach(docSnap => {
    ...
    if (canShow) {
      ...
      const timerId = `timer_${otherId}`;
      const emailDisplay = other.email ? other.email : "مستخدم";

      li.innerHTML = `
        <a href="${other.facebookPage}" target="_blank">🔗 ${emailDisplay}</a>
        <button onclick="openFacebookPage('${other.facebookPage}', '${pageId}', '${timerId}')">افتح الصفحة</button>
        <span id="${timerId}">⏳ 10 ثانية</span>
...
window.openFacebookPage = (url, buttonId, timerId) => {
  ...
  const interval = setInterval(() => {
    countdown--;
    if (timerSpan) timerSpan.innerText = `⏳ ${countdown} ثانية`;
    ...
    if (countdown <= 0) {
      ...
      if (btn) {
        ...
        if (timerSpan) timerSpan.innerText = "✅ يمكنك الآن تأكيد المتابعة";
      }
    }
  }, 1000);
};
